...
@section('pagename', 'Voucher Section')
    ...
    <div class="col-span-12">
        <div class="box">
            <div class="box-header">
                ...
                <a href="{{ route('add-coupon') }}" type="button" class="ti-btn ti-btn-success" style="float:right;">Add New
                    Event</a>
            </div>
            <div class="box-body">
                <div class="overflow-auto table-bordered">
                    <div id="coupon-table" class="ti-custom-table ti-striped-table ti-custom-table-hover tabulator"
...
@section('custom-scripts')
    <script>
        ...
        @foreach ($coupons as $coupons)
            tabledata.push({
                sl_no: {{ $coupons->id }},
                coupon_name: '{{ $coupons->voucher_name }}',
                coupon_amount: '{{ $coupons->voucher_amount }}',
                coupon_type: '{{ $coupons->discount_type }}',
                start_date: "{{ date('m-d-Y', strtotime($coupons->start_date)) }}",
                end_date: "{{ date('m-d-Y', strtotime($coupons->end_date)) }}",
        ...
        jQuery(document).ready(function($) {
            ...
            //Basic Tabulator
            var table = new Tabulator("#coupon-table", {
                width: 150,
                minWidth: 100,
                layout: "fitColumns",
                pagination: "local",
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        formatter: function(cell, formatterParams, onRendered) {
                            ...
                            if (rowData.coupon_type == 'price') {
                                var output = '$' + rowData.coupon_amount;
                            } else {
                                var output = rowData.coupon_amount + '%';
                            }
        ...
        function editAction(id) {
            window.location.href = "{{ route('modify-coupon', '') }}" + "/" + id;
        }
        ...
        function changeStatus(id, status, element) {
            const url = "{{ route('change-voucher-status') }}";
