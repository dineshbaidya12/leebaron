...
@section('title', 'NYC Calender')
@section('pagename', 'NYC Calender')
    ...
    <div class="col-span-12">
        <div class="box">
            <div class="box-header">
                ...
                <a href="{{ route('add-calender') }}" type="button" class="ti-btn ti-btn-success" style="float:right;">Add
                    New NYC Calender</a>
            </div>
            <div class="box-body">
                <div class="overflow-auto table-bordered">
                    <div id="calender-table" class="ti-custom-table ti-striped-table ti-custom-table-hover tabulator"
...
@section('custom-scripts')
    <script>
        ...
        @foreach ($nycCalenders as $nycCalender)
            tabledata.push({
                sl_no: {{ $nycCalender->id }},
                start_time: '{{ $nycCalender->start_time }}',
                end_time: '{{ $nycCalender->end_time }}',
                notes: `{!! $nycCalender->notes !!}`,
                status: '{{ $nycCalender->status }}',
            });
        @endforeach
        ...
        jQuery(document).ready(function($) {
            ...
            //Basic Tabulator
            var table = new Tabulator("#calender-table", {
                width: 150,
                minWidth: 100,
                layout: "fitColumns",
                pagination: "local",
        ...
        function editAction(id) {
            window.location.href = "{{ route('modify-calender', '') }}" + "/" + id;
        }

        function deleteAction(id, buttonElement) {
            // console.log(buttonElement);
            let table = document.getElementById('calender-table');
                    ...
                    fetch("{{ route('delete-calender') }}", {
                        ...
                        .then(response => {
                            if (response.ok) {
                                ...
                                notifier.success('NYC Calender Deleted Successfully!')
                            } else {
                                var notifier = new AWN({
                                    position: 'top-right',
                                });
        ...
        function changeStatus(id, status, element) {
            const url = "{{ route('change-calender-status', '') }}";
